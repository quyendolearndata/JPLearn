# ADR-008 — CMS draft editing and recorded Level QA

Status: integrated with the clean architecture backend on 2026-09-08; operational sign-offs remain separate.
...
- PATCH /staff/catalog/{id}: mandatory current revision (409 on conflict), draft only; at least one non-null metadata field; reject unknown fields.
...
Merge revision `0019_merge_cms_reviews` joins this history with `0018_hls_bundle_integrity`, permitting upgrades from either branch without replaying applied revisions.
...
The existing CMS list/detail UI retains draft editing and now shows review actions and history. API fixtures/E2E setup follow the same gate.
