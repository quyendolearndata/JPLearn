...
> Trạng thái: In Progress — đang thực hiện theo ghế công ty  
> Baseline: `codex/fastapi-backend-hardening`, commit `8c5515a`  
> Kế thừa: `2026-09-05-fastapi-hardening-regression-fixes.md`  
> Chủ trì: CTO; thực hiện: Platform/Backend; nghiệm thu: QA  
> Phối hợp: BA (contract), Ops (CI/release), Web (test harness)
...
- [ ] Thay tiến trình giữ lock chờ `sys.stdin.read()` bằng supervisor giữ file
      descriptor flock trong suốt vòng đời subprocess E2E và cleanup của nó.
      Không phụ thuộc terminal/stdin mở để giữ lock.
- [ ] Dùng cùng một lock ổn định cho các run có thể đụng shared workspace; chỉ
      release lock sau khi child processes đã kết thúc và cleanup hoàn tất.
- [ ] Bỏ `git checkout -- next-env.d.ts tsconfig.json` khỏi cleanup.
- [ ] Chạy build trong workspace/snapshot riêng mỗi run để Next.js có thể sửa
...
- [ ] Giữ ports, DB project, storage, build và report theo run; allocate/retry port
      sau khi nhận lock, không giữ port chưa bind trong thời gian chờ dài.
- [ ] Cleanup idempotent, giữ exit code gốc, xử lý INT/TERM và failure; chỉ dừng
      PID/process group/container do run tạo. Không chạy cleanup hai lần do ERR/EXIT.
- [ ] Lưu log/report theo run; không xóa evidence cần nghiệm thu sau khi PASS.

**Tests bắt buộc:**

- [ ] Subprocess dùng `stdin=DEVNULL`: runner A giữ lock; B không vào critical
      section cho tới khi A hoàn tất cleanup. Kiểm timestamps/barrier thay vì chỉ
      kiểm dòng log “Waiting for lock”.
- [ ] A lỗi hoặc nhận TERM: B tiếp tục, lock không bị kẹt, không còn child mồ côi.
- [ ] Trong fixture repo có uncommitted changes ở hai config files: run thành công
      và run lỗi đều giữ nguyên nội dung; thay đổi tạo trong lúc run cũng được giữ.
- [ ] Chạy hai invocation E2E thật: mỗi invocation đạt 10/10, được serialize theo
      evidence; không đụng DB/port/log của nhau.

**Exit:** lock tồn tại suốt run với stdin đóng; cleanup không có lệnh restore Git
hoặc ghi đè user files; 2-run test có artifact riêng.

## 4. R-01 — So constraint chính xác cả hai chiều

**Files:** `src/jplearn_api/openapi_diff.py`, mutation suite, OpenAPI rule docs.

- [ ] Với mục tiêu exact contract, so sự tồn tại và giá trị minLength/maxLength
      cả hai chiều. Tăng minLength hoặc giảm maxLength trên request cũng là drift.
- [ ] BA ghi policy exact-vs-compatible theo request/response nếu cần exception;
      exception phải là allowlist hẹp có lý do, không dùng logic bỏ qua chung.
- [ ] Kiểm các constraint tương tự (pattern/format/enum/additionalProperties):
...
- [ ] Mỗi mutation chạy qua CLI với spec được inject; command non-zero và diff chỉ
...
schema trước đó không có. Giữ toàn bộ mutation suite đã có.

**Exit:** mutant minLength 100 bị bắt; không còn quy tắc “chỉ cấm nới” áp chung cho
...
- [ ] Giữ key UUID riêng mỗi probe; write/read/delete đều là điều kiện healthy.
- [ ] Không nuốt unlink error. Cleanup thất bại -> unhealthy; log error class/run
      context đã redact và metric cleanup-failed; HTTP response không lộ path.
- [ ] Khi probe body lẫn cleanup cùng lỗi, giữ nguyên nguyên nhân chính và ghi rõ
      cleanup failure; không báo thành công khi còn file không dọn được.
- [ ] Giới hạn công việc I/O thực sự còn chạy, bao gồm thread sống sau timeout.
...
- [ ] Khi hết capacity, trả unhealthy trong deadline; không tạo hàng đợi vô hạn.
- [ ] Có lifecycle shutdown và cleanup khi worker hoàn tất muộn. Disk treo phải
      có giới hạn tác động lên pool chung và chính sách vận hành rõ.
- [ ] Giữ tổng timeout `/ready`; báo trạng thái từng dependency đã biết, tránh
      biến DB khỏe thành down chỉ vì storage timeout nếu đã có kết quả DB.

**Tests bắt buộc:**

- [ ] Inject unlink PermissionError -> `check_ready=False`, HTTP 503, log/metric có
      failure; file chưa xóa phải được báo, không tuyên bố zero leak giả.
- [ ] 100 probe trên storage khỏe -> 100 PASS, zero leftover.
- [ ] Chặn worker bằng threading.Event, cho nhiều request timeout/cancel liên tiếp;
...
- [ ] Read/write/fsync/readback/delete failures và shutdown có deadline đều được
      kiểm; `/health` tiếp tục giữ liveness.

**Exit:** lỗi delete không báo healthy; timeout không làm vượt giới hạn worker;
failure có thể điều tra từ log/metric.

## 6. R-07 — Upload cancellation và quyền sở hữu object

**Files:** `storage.py`, `media_service.py`, media integration tests.

- [ ] Xử lý `asyncio.CancelledError` riêng hoặc cleanup trong finally theo trạng
      thái; cleanup xong phải re-raise cancellation, không biến thành HTTP success.
- [ ] Theo dõi ownership của temp/final object và DB transaction. Stage bị hủy phải
      đóng handle và dọn `.part` sau khi pending I/O kết thúc an toàn.
- [ ] Executor open/write có thể còn chạy sau cancellation: không unlink/close
      đua với write đang chạy; không để file handle trả về muộn bị bỏ quên.
- [ ] Promote xong nhưng chưa commit: rollback DB và compensate object thuộc
...
- [ ] Nếu outcome chưa xác định, giữ object và ghi evidence/reconciliation status;
      ưu tiên tránh media row trỏ file đã bị xóa. Xác định recovery path trước khi
      đánh test cancellation-at-commit PASS.
- [ ] Cleanup có giới hạn/lifecycle phù hợp; lỗi cleanup được quan sát và không
...
và query bằng connection mới để xác nhận DB/object cuối cùng nhất quán.

**Exit:** tái hiện `cancel.part` không còn leftover ở cancel-during-stage; không
...
- [ ] CI build image từ clean checkout SHA đang review; giữ digest/image ID và SHA
      trong artifact. Không dùng tag local có sẵn làm proof của CI.
- [ ] Test wheel/package resources trong môi trường không có repo fallback;
      resource thiếu/hỏng phải fail rõ.
- [ ] Chạy cùng image non-root UID 10001; kiểm CLI help=0, unknown command!=0.
- [ ] PostgreSQL test riêng: upgrade/current, adoption baseline có dữ liệu, stamp
      empty/mismatch fail và `alembic_version` không thay khi fail.
- [ ] Container readiness: DB/storage healthy -> 200; chỉ DB hỏng hoặc chỉ storage
      read-only -> 503 với đúng dependency; health vẫn 200.
- [ ] Hoàn thành shared environment resolver app/CLI/seed còn thuộc R-08 gốc;
      missing/unknown env không mở destructive downgrade, precedence có tests.
- [ ] CI chạy guard, pytest, mutation gate và E2E theo scope; upload logs/report
      kể cả khi fail. Gate đỏ hoặc thiếu bước thì không phát hành image ready.
- [ ] Manifest có command, exit code, UTC timestamps, SHA/digest và raw artifact
      link; không chỉ viết một bảng PASS trong Markdown.

**Exit:** workflow thực sự chứa và chạy các bước trên. Nếu chưa có CI run từ remote,
ghi “implemented, remote verification pending”; không tick CI PASS bằng local run.

## 8. Tài liệu và điều kiện hoàn tất

- [ ] PR đầu sửa trạng thái R-01/R-03/R-06/R-07/R-08 trong plan cha và walkthrough
      thành reopened/partial; giữ lịch sử 128-test PASS cùng giới hạn coverage.
- [ ] Mỗi R có evidence: test đỏ baseline, fix SHA, test xanh, reviewer đúng ghế.
- [ ] BA xác nhận rule contract; QA xác nhận tests; Ops xác nhận CI/image; CTO
      xác nhận engineering acceptance sau khi các mục trên thực sự hoàn tất.
- [ ] Không tự ghi chữ ký thay người/ghế khác; dấu tick không thay evidence.
