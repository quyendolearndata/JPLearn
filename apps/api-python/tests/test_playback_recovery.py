...
async def test_start_replay_final_credit_and_terminal_retry(client_factory, postgres_url):
    ...
    item = await client.post(
        ...
        json={
            ...
            "media_type": "video",
            "visual_support": "high",
            "title_internal": "recovery",
        },
    )
    assert item.status_code == 201
    item_id = item.json()["id"]
    content = await client.put(
        f"/staff/catalog/{item_id}/content", headers=headers, json={"version_revision": 1, "scenes": []}
    )
    ...
    try:
        ...
        await conn.execute(
            "UPDATE playbacks SET created_at=$2, last_server_time=$2 WHERE id=$1",
            pid,
            (datetime.now(UTC) - timedelta(seconds=5)).replace(tzinfo=None),
        )
        final = {
            "final_seq": 1,
            "final_position_ms": 5000,
            "final_duration_ms": 60000,
            "final_client_cumulative_active_ms": 5000,
            "final_client_epoch": first.json()["epoch"],
        }
...
async def test_preferences_concurrent_update_and_effective_policy(client_factory, postgres_url):
    ...
    responses = await asyncio.gather(
        *[
            client.put(
                ...
                headers=headers,
                json={"expected_revision": 1, "daily_goal_minutes": goal, "timezone": "Pacific/Honolulu"},
            )
            for goal in (30, 120)
        ]
    )
    ...
    engine, sessions = create_engine_and_sessions(
        Settings(database_url=postgres_url, jwt_secret="test-secret-at-least-32-bytes-long-for-pyjwt-security")
    )
    try:
        async with sessions() as session:
            repo = create_uow(session).playbacks
            current = await repo.get_effective_learning_preferences(uid, datetime.now(UTC))
