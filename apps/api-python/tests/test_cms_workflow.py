...
def test_staff_list_detail_and_edit_draft(live_client):
    ...
    response = live_client.patch(
        path, headers=h, json={"revision": 1, "title_internal": "  new title  ", "ci_level": 2}
    )
    ...
    assert live_client.patch(path, headers=h, json={"revision": 3, "ci_level": 3}).status_code == 400
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"qa_round": 10},
        {"title_internal": "x" * 501},
    ],
)
def test_patch_validation(live_client, body):
    h, _ = staff(live_client)
    item_id = item(live_client, h)
    assert live_client.patch(f"/staff/catalog/{item_id}", headers=h, json={"revision": 1, **body}).status_code == 400
...
def test_learner_cannot_access_staff_workflow(live_client):
    ...
    for method, path, body in [
        ("get", "/staff/catalog", None),
        ("get", f"/staff/catalog/{item_id}", None),
