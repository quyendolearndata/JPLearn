...
from test_remediation_concurrency import client_factory, postgres_url, _create_user_with_roles


@pytest.mark.asyncio
async def test_admin_reconciles_unknown_attempt_once_over_http(
    client_factory, postgres_url,
):
    ...
    try:
        async with conn.transaction():
            await conn.execute("""INSERT INTO catalog_items(
                id,topic_id,ci_level,duration_seconds,media_type,visual_support,title_internal,created_by,status)
                VALUES($1,'daily_home',1,30,'video','high','attempt-test',$2,'draft')""", item_id, user_id)
            await conn.execute("""INSERT INTO content_versions(id,catalog_item_id,version_number)
                VALUES($1,$2,1)""", version_id, item_id)
            ...
            await conn.execute("""INSERT INTO content_jobs(
                ...
                job_id, item_id, version_id, user_id)
            await conn.execute("""INSERT INTO ai_usage_ledger(
                ...
                str(uuid4()), account_id, job_id, user_id)
            await conn.execute("""INSERT INTO ai_job_attempts(
                ...
                attempt_id, job_id, f"ai-attempt:{attempt_id}")
        ...
        assert await conn.fetchval(
            "SELECT reserved_cost_micros FROM ai_quota_accounts WHERE id=$1", account_id,
        ) == 0
        assert await conn.fetchval("""SELECT count(*) FROM ai_usage_ledger
            WHERE account_id=$1 AND kind='release'""", account_id) == 1
        assert await conn.fetchval("""SELECT status FROM ai_usage_ledger
            WHERE account_id=$1 AND kind='reservation'""", account_id) == "released"
    finally:
        await client.aclose()
        await conn.close()
