    ...
    try:
        created = await client.post(
            ...
            json={
                ...
                "media_type": "video",
                "visual_support": "high",
                "title_internal": "snapshot",
            },
        )
        assert created.status_code == 201
        item = created.json()["id"]
        media = await client.post(
            f"/staff/catalog/{item}/media",
            headers=headers,
        ...
        try:
            row = await conn.fetchrow(
                (
                    "SELECT media_asset_id,duration_seconds,duration_source,"
                    "measured_duration_ms,source_sha256 FROM content_versions WHERE id=$1"
                ),
                content.json()["id"],
            )
