...
def test_upgrade_0012_preserves_representative_learning_and_ai_state():
    ...
    async def seed_and_verify(before):
        ...
        try:
            if before:
                await conn.execute(
                    "INSERT INTO users(id,email,password_hash) VALUES ('upgrade-user',"
                    "'upgrade@example.test','not-a-login')"
                )
                await conn.execute("INSERT INTO topics(id,label_internal) VALUES ('upgrade-topic','Upgrade')")
                await conn.execute("""INSERT INTO catalog_items(id,topic_id,ci_level,duration_seconds,media_type,
                visual_support,title_internal,created_by,status)
                    VALUES ('upgrade-item','upgrade-topic',1,30,'video','high','upgrade','upgrade-user','published')""")
                await conn.execute("""INSERT INTO content_versions(id,catalog_item_id,version_number,is_published,
                is_frozen)
                ...
                await conn.execute("""INSERT INTO learning_preferences(user_id,daily_goal_minutes,timezone,revision,
                effective_at)
                ...
                await conn.execute("""INSERT INTO learner_playback_state(
                    ...
                    'web','upgrade-browser')""")
                await conn.execute("""INSERT INTO playback_checkpoints(user_id,catalog_item_id,content_version_id,
                position_ms)
                    VALUES ('upgrade-user','upgrade-item','upgrade-version',5000)""")
                await conn.execute("""INSERT INTO learner_daily_activity(user_id,date,timezone,active_ms,goal_minutes,
                goal_met)
                ...
                assert (
                    await conn.fetchval("SELECT position_ms FROM playback_checkpoints WHERE user_id='upgrade-user'")
                    == 5000
                )
                ...
                assert (
                    playback["status"] == "active" and playback["last_seq"] == 7 and playback["total_active_ms"] == 5000
                )
                ...
                assert (
                    await conn.fetchval("SELECT count(*) FROM content_versions WHERE catalog_item_id='upgrade-item'")
                    == 1
                )
