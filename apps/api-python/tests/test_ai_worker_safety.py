...
from jplearn_api.application.commands import CreateContentJobCommand, CancelContentJobCommand, ApplyContentJobCommand
from jplearn_api.application.handlers.content_jobs import (
    handle_create_content_job, handle_cancel_content_job, handle_execute_ai_worker_step, handle_apply_content_job,
)
...
from test_content_jobs import base_test_environment
...
async def test_missing_provider_fails_before_claim(monkeypatch, environment, trial):
    monkeypatch.setattr(ai_worker, "get_settings", lambda: SimpleNamespace(
        staff_ai_enabled=True, environment=environment, enable_trial_transcriber=trial,
    ))
    monkeypatch.setattr(ai_worker, "create_engine_and_sessions", lambda _: pytest.fail("DB opened"))
...
async def test_persistence_claim_only_selects_queued():
    ...
    await SqlAlchemyContentJobRepository(session).claim_next_queued_job(datetime.now(timezone.utc), 60, "token")
...
async def test_cancel_during_provider_success_still_records_cost(base_test_environment):
    ...
    uow = env["uow_factory"]()
    job = await handle_create_content_job(CreateContentJobCommand(
        catalog_item_id=env["catalog_id"], content_version_id=env["version_id"],
        task="transcript", language="ja", idempotency_key="late-cost", user_id=env["user_id"],
        estimated_audio_seconds=120, estimated_cost_micros=300000,
    ), uow)

    class CancelDuringCall(FakeAiTranscriptionPort):
        async def transcribe_and_segment(self, **kwargs):
            await handle_cancel_content_job(CancelContentJobCommand(
                job_id=job.id, user_id=env["user_id"], user_roles=("teacher",),
            ), uow)
...
async def test_cancel_then_timeout_keeps_unknown_reservation(base_test_environment):
    ...
    job = await handle_create_content_job(CreateContentJobCommand(
        catalog_item_id=env["catalog_id"], content_version_id=env["version_id"],
        task="transcript", language="ja", idempotency_key="cancel-timeout", user_id=env["user_id"],
        estimated_audio_seconds=120, estimated_cost_micros=300000,
    ), uow)
...
async def test_outcome_updates_detached_reservation_model():
    ...
    session = SimpleNamespace(
        ...
        add=lambda entry: None, flush=AsyncMock(),
    )
...
async def test_apply_failure_rolls_back_transcript_and_job(base_test_environment):
    env = base_test_environment
    uow = env["uow_factory"]()
    dto = await handle_create_content_job(CreateContentJobCommand(
        catalog_item_id=env["catalog_id"], content_version_id=env["version_id"],
        task="transcript", language="ja", idempotency_key="atomic-apply", user_id=env["user_id"],
        estimated_audio_seconds=120, estimated_cost_micros=300000,
    ), uow)
    async with uow:
    ...
    with pytest.raises(RuntimeError, match="write failed"):
        await handle_apply_content_job(ApplyContentJobCommand(
            job_id=dto.id, expected_revision=1, user_id=env["user_id"],
        ), uow)
...
async def test_unknown_attempt_can_be_reconciled_not_billed_exactly_once(base_test_environment):
    ...
    job = await handle_create_content_job(CreateContentJobCommand(
        catalog_item_id=env["catalog_id"], content_version_id=env["version_id"],
        task="transcript", language="ja", idempotency_key="reconcile-not-billed",
        user_id=env["user_id"], estimated_audio_seconds=120,
        estimated_cost_micros=300000,
    ), uow)
    await handle_execute_ai_worker_step(
        uow, FakeAiTranscriptionPort(exception_to_raise=TimeoutError("unknown outcome")),
        attempt_token="attempt-not-billed",
    )

    request = dict(
        job_id=job.id, attempt_id="attempt-not-billed", user_id=env["user_id"],
        user_roles=("admin",), decision="not_billed",
    ...
    with pytest.raises(ConflictError, match="already reconciled"):
        await reconcile_ai_attempt(
            uow, **{**request, "evidence": "Different provider evidence for the same attempt."},
        )
...
async def test_unknown_attempt_can_be_reconciled_with_actual_billed_usage(base_test_environment):
    ...
    job = await handle_create_content_job(CreateContentJobCommand(
        catalog_item_id=env["catalog_id"], content_version_id=env["version_id"],
        task="transcript", language="ja", idempotency_key="reconcile-billed",
        user_id=env["user_id"], estimated_audio_seconds=120,
        estimated_cost_micros=300000,
    ), uow)
    await handle_execute_ai_worker_step(
    ...
    attempt = await reconcile_ai_attempt(
        uow, job_id=job.id, attempt_id="attempt-billed", user_id=env["user_id"],
        user_roles=("admin",), decision="billed",
        evidence="Provider invoice confirms this request and its measured usage.",
        provider_request_id="provider-request-recovered",
        usage={"audio_seconds": 118, "input_tokens": 90, "output_tokens": 20,
               "cost_micros": 125000, "currency": "USD"},
    )
...
async def test_expired_attempt_is_quarantined_without_retry_or_quota_release(base_test_environment):
    ...
    dto = await handle_create_content_job(CreateContentJobCommand(
        catalog_item_id=env["catalog_id"], content_version_id=env["version_id"],
        task="transcript", language="ja", idempotency_key="expired-attempt",
        user_id=env["user_id"], estimated_audio_seconds=120,
        estimated_cost_micros=300000,
    ), uow)
    ...
    async with uow:
        ...
        await uow.content_jobs.save_attempt(AiAttempt(
            id="expired-token", job_id=job.id, attempt_number=job.attempt,
            provider="test", idempotency_key="ai-attempt:expired-token", state="running",
            lease_expires_at=now - timedelta(seconds=1), created_at=now - timedelta(seconds=61),
