...
def split_delta_by_timezone(
    ...
    ms_before_midnight = max(0, int((midnight_local.astimezone(timezone.utc) - start_time.astimezone(timezone.utc)).total_seconds() * 1000))
...
async def handle_start_playback(
    ...
    async with uow:
        ...
        state = await uow.playbacks.acquire_learner_playback_lock(
            cmd.user_id, device_class=device_class, client_instance_id=client_instance_id,
        )
        ...
        if cmd.idempotency_key:
            ...
            if saved:
                ...
                if cutoff is not None and previous.created_at.replace(tzinfo=timezone.utc) <= cutoff.replace(tzinfo=timezone.utc):
        ...
        if resume_cp is not None and cutoff is not None:
            resume_time = resume_cp.updated_at if resume_cp.updated_at.tzinfo else resume_cp.updated_at.replace(tzinfo=timezone.utc)
            cutoff_time = cutoff if cutoff.tzinfo else cutoff.replace(tzinfo=timezone.utc)
        ...
        if cmd.idempotency_key:
            await uow.playbacks.save_start_receipt(cmd.user_id, cmd.idempotency_key, fingerprint,
...
async def handle_send_checkpoint(
    ...
    uow: AsyncUnitOfWork,
    *, credit_enabled: bool = True, auto_commit: bool = True,
    ...
    async with uow:
        ...
        # 3. Sequence & Idempotency check with receipt (P1.7: Check receipt BEFORE live-state to allow idempotent retries)
        ...
        last_time_aware = last_server_time if last_server_time.tzinfo else last_server_time.replace(tzinfo=timezone.utc)
        ...
        if server_elapsed_ms > gap_threshold_ms:
            ...
            started = session.created_at.replace(tzinfo=timezone.utc) if session.created_at.tzinfo is None else session.created_at
            remaining_wall_ms = max(0, int((now - started).total_seconds() * 1000) + CLOCK_SKEW_TOLERANCE_MS - session.total_active_ms)
        ...
        if accepted_delta_ms > 0:
            ...
            boundary = final.effective_at.replace(tzinfo=timezone.utc) if final.effective_at.tzinfo is None else final.effective_at
            first = final
            ...
            if last_time_aware < boundary < now and (first.timezone, first.daily_goal_minutes) != (final.timezone, final.daily_goal_minutes):
                before = int(accepted_delta_ms * (boundary-last_time_aware).total_seconds() / (now-last_time_aware).total_seconds())
                ...
                for date_str, alloc_ms in split_delta_by_timezone(start, end, amount, pref.timezone):
                    if alloc_ms > 0:
                        await uow.playbacks.record_daily_active_ms(
                            user_id=cmd.user_id, date_str=date_str, timezone_str=pref.timezone,
                            delta_ms=alloc_ms, goal_minutes=pref.daily_goal_minutes,
    ...
    *, credit_enabled: bool = True,
    ...
    async with uow:
        ...
        if session is None or session.user_id != cmd.user_id:
            raise EntityNotFoundError("Playback session not found")
        state = await uow.playbacks.acquire_learner_playback_lock(
            cmd.user_id, device_class=session.device_class,
        ...
        now = datetime.now(timezone.utc)
        cutoff = await uow.playbacks.get_latest_history_deletion_cutoff(cmd.user_id)
        last = session.last_server_time.replace(tzinfo=timezone.utc) if session.last_server_time.tzinfo is None else session.last_server_time
        ...
        if cutoff is not None and last <= cutoff:
            ...
            if cmd.final_seq is not None and cmd.final_position_ms is not None and state.is_lease_active(now):
                _, session = await handle_send_checkpoint(
                    SendCheckpointCommand(
                        user_id=cmd.user_id, playback_id=cmd.playback_id,
                        seq=cmd.final_seq, position_ms=cmd.final_position_ms,
                        duration_ms=cmd.final_duration_ms or 0,
                        playback_rate=cmd.final_playback_rate, state="ended",
                        client_cumulative_active_ms=(cmd.final_client_cumulative_active_ms
                            if cmd.final_client_cumulative_active_ms is not None else session.last_client_cumulative_ms),
                        client_epoch=session.epoch, scene_id=cmd.final_scene_id,
                    ), uow, credit_enabled=credit_enabled, auto_commit=False,
                )
...
async def handle_get_playback(
    ...
    async with uow:
        ...
        now = datetime.now(timezone.utc)
        last_time_aware = session.last_server_time if session.last_server_time.tzinfo else session.last_server_time.replace(tzinfo=timezone.utc)
