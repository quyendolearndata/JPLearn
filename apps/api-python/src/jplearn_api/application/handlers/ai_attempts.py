    ...
    for attempt_id in ids:
        async with uow:
            ...
            if attempt is None or attempt.state != "running":
                continue
            expiry = attempt.lease_expires_at.replace(tzinfo=UTC)
            if expiry > now:
                continue
            ...
            if job is not None and job.attempt_token == attempt.id and job.status.value == "running":
                job.record_failure(
                    "Provider outcome unknown after lease expiry; reconcile attempt", retryable=False, now=now
                )
    ...
    async with uow:
        ...
        if decision == "billed":
            entry = account.settle(
                reservation,
                *(usage[k] for k in units),
                provider_request_id=provider_request_id,
                attempt=attempt.attempt_number,
                now=now,
                description="Operator confirmed billed usage",
            )
