...
class Settings(BaseSettings):
    ...
    def validate_configuration(self) -> Settings:
        ...
        if len(self.jwt_secret.encode("utf-8")) < 32:
            raise ValueError(f"JWT_SECRET must be at least 32 bytes (got {len(self.jwt_secret.encode('utf-8'))} bytes)")
        ...
        if self.storage_root is not None:
            ...
            if not storage_path.is_absolute():
                raise ValueError(f"STORAGE_ROOT must be an absolute path, got '{self.storage_root}'")
        ...
        if self.environment in ("staging", "production"):
            if not self.api_public_url or not self.api_public_url.startswith("https://"):
                raise ValueError(f"API_PUBLIC_URL must use HTTPS in {self.environment} environment")
            if not self.media_signing_secret:
                raise ValueError(f"MEDIA_SIGNING_SECRET is required and cannot be empty in {self.environment}")
            if self.media_signing_secret == self.jwt_secret:
                raise ValueError(f"MEDIA_SIGNING_SECRET must be distinct from JWT_SECRET in {self.environment}")
            ...
            if set(self.cors_origins) == default_localhost_origins:
                raise ValueError(
                    f"Explicit HTTPS CORS_ORIGINS required in {self.environment} "
                    "(cannot inherit default localhost origins)"
                )

            if not self.cors_origins or any(o.strip() in ("*", "null") for o in self.cors_origins):
                raise ValueError(f"CORS_ORIGINS cannot be empty, null, or contain wildcard '*' in {self.environment}")

            for origin in self.cors_origins:
                parsed = urlparse(origin)
                if parsed.scheme != "https":
                    raise ValueError(f"CORS_ORIGINS entries must use HTTPS in {self.environment}, got '{origin}'")
                if not parsed.netloc:
                    raise ValueError(f"Invalid CORS origin netloc in {self.environment}: '{origin}'")
                if parsed.path not in ("", "/"):
                    raise ValueError(f"CORS origin must not include a path in {self.environment}: '{origin}'")
                if parsed.params or parsed.query or parsed.fragment:
                    raise ValueError(
                        f"CORS origin must not include query or fragment in {self.environment}: '{origin}'"
                    )
                if "@" in parsed.netloc:
                    raise ValueError(f"CORS origin must not contain user credentials in {self.environment}: '{origin}'")
            ...
            if self.cors_origin_regex is not None:
                ...
                if any(tok in self.cors_origin_regex for tok in broad_tokens):
                    raise ValueError(
                        f"Broad or unapproved CORS_ORIGIN_REGEX rejected in "
                        f"{self.environment}: '{self.cors_origin_regex}'"
                    )

            if "dev-secret" in self.jwt_secret.lower() or "change-me" in self.jwt_secret.lower():
                raise ValueError(f"Insecure JWT_SECRET detected in {self.environment} environment")
