...
def upgrade() -> None:
    ...
    op.execute(
        "ALTER TABLE learner_daily_activity ADD CONSTRAINT "
        "learner_daily_activity_pkey PRIMARY KEY (user_id, date, policy_revision)"
    )
...
def downgrade() -> None:
    ...
    op.execute(
        "ALTER TABLE learner_daily_activity ADD CONSTRAINT learner_daily_activity_pkey PRIMARY KEY (user_id, date)"
    )
