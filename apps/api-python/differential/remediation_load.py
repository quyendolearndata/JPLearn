...
def latency_summary(values):
    ...
    return {
        "samples": len(ordered),
        "p50_ms": ordered[int(len(ordered) * 0.50) - 1],
...
async def sample_runtime(url, pool, stop, metrics):
    ...
    try:
        while not stop.is_set():
            ...
            metrics["max_active_connections"] = max(
                metrics["max_active_connections"], int(row["active_connections"] or 0)
            )
            if checked_out >= pool.size():
                metrics["pool_saturated_samples"] += 1
            lock_waiters = int(row["lock_waiters"] or 0)
            metrics["max_lock_waiters"] = max(metrics["max_lock_waiters"], lock_waiters)
            if lock_waiters:
                metrics["lock_wait_samples"].append({"at": datetime.now(UTC).isoformat(), "waiters": lock_waiters})
            try:
...
async def query_plans(conn, user, clip, version, today):
    return {
        "published_content_version": await explain(
            conn,
        ...
        "content_scenes": await explain(
            conn, "SELECT id FROM scenes WHERE content_version_id=$1 ORDER BY scene_index", version
        ),
        "daily_activity": await explain(
            conn,
            """SELECT * FROM learner_daily_activity
            WHERE user_id=$1 AND date >= $2 AND date <= $2 ORDER BY date""",
            user,
            today,
        ),
        "scene_search_candidates": await explain(
            conn,
            """SELECT ast.id FROM approved_scene_texts ast
            ...
            ORDER BY ast.catalog_item_id,ast.scene_index""",
            "負荷検索0000-00",
        ),
        "playback_owner": await explain(conn, "SELECT * FROM learner_playback_state WHERE user_id=$1", user),
        "watch_history": await explain(
            conn,
            """SELECT id FROM playbacks WHERE user_id=$1
...
async def workload(url, mode, pool_size=50):
    ...
    try:
        await conn.executemany(
            "INSERT INTO users(id,email,password_hash) VALUES ($1,$2,'unused')", [(u, f"{u}@load.test") for u in users]
        )
        await conn.executemany(
            "INSERT INTO user_roles(user_id,role) VALUES ($1,$2)",
            [(u, r) for u in users for r in ("learner", "teacher")],
        )
        await conn.executemany(
            (
                "INSERT INTO learner_progress(user_id,minutes_comprehensible,"
                "current_ci_level,updated_at) VALUES ($1,0,1,now())"
            ),
            [(u,) for u in users],
        )
        await conn.execute("INSERT INTO topics(id,label_internal) VALUES ('load','load')")
        await conn.executemany(
            """INSERT INTO catalog_items(id,topic_id,ci_level,duration_seconds,media_type,visual_support,title_internal,
            created_by,status)
            VALUES ($1,'load',1,300,'video','high','load',$2,'published')""",
            [(c, users[0]) for c in clips],
        )
        await conn.executemany(
            """INSERT INTO media_assets(id,catalog_item_id,storage_key,mime,measured_duration_ms,source_sha256)
            VALUES ($1,$2,$3,'video/mp4',300000,$4)""",
            [(m, c, f"load/{m}.mp4", f"{i:064x}") for i, (m, c) in enumerate(zip(media, clips, strict=False), 1)],
        )
        await conn.executemany(
            ...
            [
                (v, c, m, f"load/{m}.mp4", f"{i:064x}")
                for i, (v, c, m) in enumerate(zip(versions, clips, media, strict=False), 1)
            ],
        )
        await conn.executemany(
            """INSERT INTO scenes(id,content_version_id,scene_index,start_time_seconds,end_time_seconds,title_jp,
            transcript_jp)
            VALUES ($1,$2,$3,$4,$5,'店',$6)""",
            scene_rows,
        )
        ...
        await conn.executemany(
            """INSERT INTO approved_scene_texts(
        ...
        await conn.executemany(
            """INSERT INTO ai_quota_accounts(id,user_id,name)
            VALUES ($1,$2,'load')""",
            [(str(uuid4()), u) for u in users],
        )
        await conn.execute(
            """INSERT INTO playbacks(id,user_id,catalog_item_id,content_version_id,device_class,client_instance_id,
            status)
            SELECT md5('history-'||g)::uuid::text,$1,$2,$3,'web','historical','completed' FROM generate_series(1,100000)
            g""",
            users[0],
            clips[0],
            versions[0],
        ...
        deadlocks_before = int(
            await conn.fetchval("SELECT deadlocks FROM pg_stat_database WHERE datname=current_database()") or 0
        )
        correctness = {}
        async with lifespan(app):
            ...
            def timed_do_get():
                ...
                try:
                    ...
                    pool_acquire_ms.append((time.perf_counter() - began) * 1000)

            pool._do_get = timed_do_get
            async with (
                runtime_monitor(url, pool, runtime_metrics),
                AsyncClient(transport=ASGITransport(app=app), base_url="http://load") as client,
            ):
                ...
                async def learner(i):
                    ...
                    start = await client.post(
                        "/playbacks",
                        headers=headers,
                        json={"catalog_item_id": clips[i], "content_version_id": versions[i], "device_id": f"load-{i}"},
                    )
                    assert start.status_code == 201, start.text
                    pid = start.json()["playback_id"]
                    epoch = start.json()["epoch"]
                    for seq in range(1, 5):
                        await asyncio.sleep(15)
                        began = time.perf_counter()
                        result = await client.put(
                            f"/playbacks/{pid}/checkpoints/{seq}",
                            headers=headers,
                            json={
                                "position_ms": seq * 15000,
                                "duration_ms": 300000,
                                "playback_rate": 1,
                                "state": "playing",
                                "client_cumulative_active_ms": seq * 15000,
                                "client_epoch": epoch,
                            },
                        )
                        samples.append(
                    ...
                    job = await client.post(
                        f"/staff/catalog/{clips[i]}/content-jobs",
                        headers={**headers, "Idempotency-Key": f"load-job-{i}"},
                        json={"content_version_id": versions[i], "task": "transcript", "language": "ja"},
                    )
                    samples.append(
                        {
                ...
                credit_rows = await conn.fetch(
                    """SELECT user_id,sum(active_ms)::bigint AS active_ms
                    FROM learner_daily_activity WHERE user_id=ANY($1::text[]) GROUP BY user_id""",
                    users,
                )
                actual_credit = {row["user_id"]: int(row["active_ms"]) for row in credit_rows}
                credit_mismatches = {
                    user: actual_credit.get(user, 0) for user in users if actual_credit.get(user, 0) != 60000
                }
                receipt_credit = int(
                    await conn.fetchval(
                        """SELECT coalesce(sum(accepted_delta_ms),0)
                ...
                correctness = {
                    "checkpoint_credit_expected_ms": 6000000,
        ...
        runtime_metrics["pool_saturation_ratio"] = (
            runtime_metrics["pool_saturated_samples"] / runtime_metrics["sample_count"]
            if runtime_metrics["sample_count"]
            else 0
        )
        summary = {}
        ...
        out = Path("../../docs/qa/evidence/backend-remediation")
        out.mkdir(parents=True, exist_ok=True)
        suffix = "" if pool_size == 50 else f"-pool{pool_size}"
        (out / f"load-{mode}{suffix}-candidate.json").write_text(
            json.dumps(
                {
                    ...
                    (
                        "scope"
                    ): f"ASGI; no network latency claim; 4 rounds of 15s heartbeats; 100-client {mode}; candidate only",
                    "pool": {
                        "size": pool_size,
                        "max_overflow": 0,
                        "pre_ping": False,
                        ...
                        "samples_ms": pool_acquire_ms,
                    },
                    "dataset": {
                        "clips": 1000,
                        "scenes": 10000,
                        "approved_scene_texts": 10000,
                        "historical_playbacks": 100000,
                        "concurrent_playbacks": 100,
                    },
            ...
            json.dumps(
                {
                    "summary": summary,
                    "pool": pool_summary,
                    "runtime": {key: value for key, value in runtime_metrics.items() if key != "lock_wait_samples"},
                    "correctness": correctness,
                }
            )
        )
...
if __name__ == "__main__":
    ...
    try:
        asyncio.run(workload(url, args.mode, args.pool_size))
    finally:
